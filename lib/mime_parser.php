...
{/metadocument}
 */
    public $message_buffer_length = 8000;
 ...
 */
    ...
    /* Private variables */
    public $state           = MIME_PARSER_START;
    public $buffer          = '';
    public $buffer_position = 0;
    public $offset          = 0;
    public $parts           = array();
    public $part_position   = 0;
    public $headers         = array();
    public $body_parser;
    public $body_parser_state    = MIME_PARSER_BODY_DONE;
    public $body_buffer          = '';
    public $body_buffer_position = 0;
    public $body_offset          = 0;
    public $current_header       = '';
    public $file;
    public $body_file;
    public $position               = 0;
    public $body_part_number       = 1;
    public $next_token             = '';
    public $lines                  = array();
    public $line_offset            = 0;
    public $last_line              = 1;
    public $last_carriage_return   = 0;
    public $header_name_characters = '!"#$%&\'()*+,-./0123456789;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\]^_`abcdefghijklmnopqrstuvwxyz{|}';
    public $message_position       = 0;
     ...
     * @param $error
     */
    public function SetErrorWithContact($error)
    {
     ...
     * @param $type
     * @param $parameters
     * @param $character_sets
     * @param $languages
     */
    public function ParseStructuredHeader($value, &$type, &$parameters, &$character_sets, &$languages)
    {
     ...
     * @param $line_break
     */
    public function FindStringLineBreak($string, $position, &$break, &$line_break)
    {
     ...
     * @param $line_break
     */
    public function FindLineBreak($position, &$break, &$line_break)
    {
     ...
     * @param $line_break
     */
    public function FindBodyLineBreak($position, &$break, &$line_break)
    {
     ...
     * @param $headers
     */
    public function ParseHeaderString($body, &$position, &$headers)
    {
        $l       = strlen($body);
        $headers = array();
        for (; $position < $l;)
     ...
     * @param $end
     * @param $part
     * @param $need_more_data
     */
    public function ParsePart($end, &$part, &$need_more_data)
    {
        $need_more_data = 0;
        switch ($this->state)
        {
    ...
    public function QueueBodyParts()
    {
        for (;;)
        {
     ...
     * @param $return
     */
    public function ParseParameters($value, &$first, &$parameters, $return)
    {
     ...
     * @param $offset
     */
    public function ParseBody($data, $end, $offset)
    {
     ...
     * @param $part
     */
    public function DecodePart($part)
    {
        switch ($part['Type'])
        {
            case 'MessageStart':
     ...
     * @param $parameters
     * @param $position
     * @param $end_of_message
     * @param $decoded
     */
    public function DecodeStream($parameters, $position, &$end_of_message, &$decoded)
    {
 ...
 */
     ...
     * @return null
     */
    public function CopyAddresses($message, &$results, $header)
    {
 ...
 */
     ...
     * @param $column
     */
    public function GetPositionLine($position, &$line, &$column)
    {
        if (!$this->track_lines)
        {
